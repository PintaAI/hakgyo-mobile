...
Track user's daily app opens to award login XP and maintain streak.
...
- The `DAILY_LOGIN` event awards **5 XP** for the first login of the day
- Streak is incremented on first activity each day
- Streak resets if user is inactive for 24+ hours
- The API handles duplicate login events (only first login per day counts)
- XP is multiplied by streak bonus: 1.0x (0-2 days), 1.25x (3-6 days), 1.5x (7+ days)
